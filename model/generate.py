...
ideal_health = { 
    "high blood pressure": 0,             # 0 = No high BP, 1 = high BP
    "high cholesterol": 0,                # 0 = No high cholesterol, 1 = high cholesterol
    "cholesterol check": 0,               # 0 = No cholesterol check in last 5y, 1 = cholesterol check in last 5 years (Discard)
    "bmi": 25,                            # Body mass index (weight/height^2)
    "smoker": 0,                          # 0 = Non smoker, 1 = smoker
    "history of stroke": 0,               # 0 = No history of stroke, 1 = history of stroke
    "history of heart issues": 0,         # 0 = No history of heart disease or heart attacks, 1 = history of heart disease/attack
    "lack of physical activity": 1,       # 0 = No physical activity in the past 30 days, 1 = physical activity in past 30 days
    "inadequate fruit intake": 1,         # 0 = Does not consume 1 or more fruits per day, 1 = consumes fruit 1 or more times per day
    "inadequate vegetable intake": 1,     # 0 = Does not consume 1 or more veggies per day, 1 = consumes 1 or more veggies per day
    "excess alcohol consumption": 0,      # 0 = No heavy alcohol consumption, 1 = heavy alcohol consumption (14+ drinks/week for men, 7+ drinks/week for women)
    "access to healthcare": 1,            # 0 = No healthcare plan, 1 = has a healthcare plan
    "general health": 1,                  # 1 = Excellent health, 2 = very good health, 3 = good health, 4 = fair health, 5 = poor health
    "mental health": 0,                   # How many of the past 30 days has a mental health issue affected you?
